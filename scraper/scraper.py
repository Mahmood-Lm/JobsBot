...
async def get_jobs_async(context, search_url):
    """Opens a tab to scrape the initial job search results."""
    jobs_found = []
    page = await context.new_page() # Open a new tab
    page.set_default_timeout(5000)

    async def _safe_text(locator, timeout_ms=2000):
        try:
            value = await locator.first.text_content(timeout=timeout_ms)
            return value.strip() if value else None
        except AsyncPlaywrightTimeoutError:
            return None

    async def _safe_attr(locator, attr, timeout_ms=2000):
        try:
            return await locator.first.get_attribute(attr, timeout=timeout_ms)
        except AsyncPlaywrightTimeoutError:
            return None
    ...
    try:
        ...
        for i in range(card_count):
            try:
                ...
                for attempt in range(2):
                    ...
                    title = await _safe_text(card.locator('.base-search-card__title'))
                    company = await _safe_text(card.locator('.base-search-card__subtitle'))
                    link = await _safe_attr(card.locator('a.base-card__full-link'), 'href')
        ...
        await page.close() # CRITICAL: Close the tab to free up memory!
        
    return jobs_found


async def get_job_description_async(context, job_url, timeout_ms=15000):
    """Opens a tab to extract the full description text of a specific job."""
    page = await context.new_page() # Open a new tab
    ...
    try:
        ...
        elapsed_ms = int((time.monotonic() - start_time) * 1000)
        remaining_ms = max(500, timeout_ms - elapsed_ms)
        await page.wait_for_selector(selectors, timeout=remaining_ms)
        description = await page.locator(selectors).first.inner_text()
        logger.info("Successfully scraped job description", extra={"job_url": job_url[:50]})
        return description, False
    except AsyncPlaywrightTimeoutError:
        return "Description not available.", True
        ...
        await page.close() # CRITICAL: Close the tab!
